`baseline` <-
function(spect, init.bd, sm.par = 1e-11, sm.ord = 2, max.iter = 40, tol = 5e-8,
    ...
    neg.norm.by <- substr(neg.norm.by[1],1,1)
    sm.norm.by <- substr(sm.norm.by[1],1,1)
    if(sm.norm.by == "c" || neg.norm.by == "c"){
